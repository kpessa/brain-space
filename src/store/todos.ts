...
interface TodoStore {
  ...
  recordAttempt: (
    todoId: string,
    attempt: Omit<TodoAttempt, 'id' | 'todoId' | 'createdAt'>
  ) => Promise<void>
...
export const useTodoStore = create<TodoStore>((set, get) => ({
  ...
  fetchTodos: async (userId: string) => {
    ...
    try {
      ...
      if (sort.field === 'priority') {
        ...
        query = query
          .order('priority_importance', { ascending: sort.direction === 'asc', nullsFirst: false })
    ...
    try {
      ...
      set(state => ({
        todos: state.todos.map(todo =>
          todo.id === todoId ? { ...todo, ...transformUpdateInput(updates) } : todo
        ),
      }))
    } catch (error) {
  ...
  batchUpdateTodos: async updates => {
    try {
      ...
      await Promise.all(updates.map(({ id, changes }) => get().updateTodo(id, changes)))
    } catch (error) {
  ...
  makeRecurring: async (
    ...
    try {
      const { error } = await supabase.from('todo_recurrence').insert({
        todo_id: todoId,
  ...
  removeRecurrence: async (todoId: string) => {
    try {
      const { error } = await supabase.from('todo_recurrence').delete().eq('todo_id', todoId)
      ...
      set(state => ({
        todos: state.todos.map(todo =>
          todo.id === todoId ? { ...todo, recurrence: undefined } : todo
        ),
      }))
    } catch (error) {
  ...
  recordAttempt: async (
    ...
    try {
      const { error } = await supabase.from('todo_attempts').insert({
        todo_id: todoId,
  ...
  getTodoStats: async (userId: string) => {
    try {
      ...
      const stats: TodoStats = {
        ...
        overdueCount: todos.filter(
          t => t.status === 'pending' && t.dueDate && new Date(t.dueDate) < new Date()
        ).length,
        todayCount: todos.filter(t => t.scheduledDate === new Date().toISOString().split('T')[0])
          .length,
  ...
  linkToBrainDump: async (todoId: string, braindumpId: string, nodeId: string) => {
    try {
      const { error } = await supabase.from('braindump_todos').insert({
        todo_id: todoId,
  ...
  linkToJournal: async (todoId: string, journalEntryId: string, type: string) => {
    try {
      const { error } = await supabase.from('journal_todos').insert({
        todo_id: todoId,
  ...
  linkToRoutine: async (todoId: string, routineEntryId: string, type: string) => {
    try {
      const { error } = await supabase.from('routine_todos').insert({
        todo_id: todoId,
...
function transformTodoFromDB(dbTodo: TodoWithRelations): Todo {
  ...
  return {
    ...
    recurrence: dbTodo.recurrence?.[0]
      ? transformRecurrenceFromDB(dbTodo.recurrence[0])
      : undefined,
