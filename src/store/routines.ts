    ...
    try {
      ...
      if (useFirebaseAuth()) {
        ...
        if (progress.currentDay > 0) {
          const currentDayEntry = entries.find(
            (e: RoutineEntry) => e.dayNumber === progress.currentDay
          )
          if (currentDayEntry) {
            set({ currentEntry: currentDayEntry })
          }
  ...
  completeMorning: async data => {
    ...
    if (progress && currentEntry) {
      ...
      const updatedProgress = {
        ...
        totalDaysCompleted: bothCompleted
        ...
        longestStreak: Math.max(
          ...
          progress.currentStreak + (bothCompleted ? 1 : 0)
        ),
      }
