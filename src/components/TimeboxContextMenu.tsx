...
import { X, CheckCircle, RotateCcw, Clock, FileText, Calendar, Plus } from 'lucide-react'
...
export function TimeboxContextMenu({ 
  ...
  const initialLinearImportance = task.importance !== undefined ? Math.round(logToLinear(task.importance)) : 5
  const initialLinearUrgency = task.urgency !== undefined ? Math.round(logToLinear(task.urgency)) : 5
        ...
        <div>
          ...
          {task.dueDate && (
            <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
              ...
              <span>Due: {format(new Date(task.dueDate), 'MMM d')} - {getDueDateDescription(task.dueDate)}</span>
            </div>
          )}
        </div>
