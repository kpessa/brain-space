...
import type { TimeSlot } from '@/store/useTimeboxStore'

interface TimeboxSlotContextMenuProps {
  isOpen: boolean
...
export function TimeboxSlotContextMenu({
  isOpen,
  position,
  slot,
  selectedDate,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (inputValue.trim() && selectedTemplate) {
      ...
      if (selectedTemplate.type === 'category') {
        onAddNode(inputValue.trim(), 'category', inputValue.trim().toLowerCase(), timeboxInfo)
      } else {
        onAddNode(inputValue.trim(), selectedTemplate.type, selectedTemplate.category, timeboxInfo)
      }
      onClose()
      setShowInput(false)
      setInputValue('')
          ...
          <>
            ...
            <div className="flex items-center justify-between p-3 border-b bg-gray-50">
              <h3 className="font-semibold text-sm flex items-center gap-2">
                ...
                {selectedTemplate?.label} at {slot.displayTime}
              </h3>
