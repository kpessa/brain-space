import type { RecurrencePattern, RecurringCompletion, BrainDumpNode } from '@/types/braindump'
...
export function shouldTaskOccurOnDate(
  ...
  switch (pattern.type) {
    ...
    case 'weekly': {
      const frequency = pattern.frequency || 1
      const weeksDiff = Math.floor((checkDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24 * 7))
...
export function generateRecurringTaskInstances(
  ...
  nodes.forEach(node => {
    // Only process recurring tasks and habits
    if (node.data.taskType === 'recurring' || node.data.taskType === 'habit') {
      if (node.data.recurrencePattern && shouldTaskOccurOnDate(node.data.recurrencePattern, targetDate)) {
        const completion = getRecurringTaskCompletionForDate(node.data.recurringCompletions, targetDate)
        instances.push({
          node,
