...
export function generateRecurringTaskInstances(
  ...
  nodes.forEach(node => {
    ...
    if (node.data.taskType === 'recurring' || node.data.taskType === 'habit') {
      if (
        node.data.recurrencePattern &&
        shouldTaskOccurOnDate(node.data.recurrencePattern, targetDate)
      ) {
        const completion = getRecurringTaskCompletionForDate(
          node.data.recurringCompletions,
          targetDate
        )
        instances.push({
          node,
